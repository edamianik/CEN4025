...
public class JUnitJavadocValidationUtility2
{
	 ...
	 * 
	 * @param list List of files to evaluate
	 */
	...
	/**
	 * Packages names that are sub-packages (e.g. week01.app)
	 * need to be parsed and changed into folder paths to create a valid
	 * file path. 
	 * If the package name has no sub-packages, it is returned unmodified.
	 * Used by @see initFilePath
	 * 
	 * @param packageName The package name to change into a path.
	 * @return
	 */
	private String buildPathFromPackage(String packageName)
	{
		String pathSeparator = File.separator;
		String normalizedPath = packageName + File.separator; // assume no subpackages
	...
	private boolean evaluateFileToken(FileToken fileToken, FileTestData2 testFile)
	{
	...
	private boolean verifyFileNameMatch(FileToken fileToken, FileTestData2 testFile)
	{
	 ...
	 * 
	 * @param packageName package name to remove semi-colon from
		...
		for(MethodTestData2 expectedMethod : expectedMethodsList)
		{
			...
			else
			{
				...
				String msg = String.format("    -- ** %s: Missing method - %s **",
						fileToken.getClassToken().getClassName(), expectedMethod.getMethodName()+"("+expectedMethod.getParameterTypes()+")");
