...
public class ClassToken2
{
	public ClassToken2(String className, CommentToken comment)
	{
		m_comment = comment;
	 ...
	 * @param methodName
	 *            The name of the method to locate
	 * @param paramTypes String of expected parameter types - concatenated left to right           
	 * @return MethodToken reference
	 */
	...
	/**
	 * Maps the emthods for easy access The key is the emthod name + the
	...
	private void mapMethods()
	{
		...
		{
			String suffix = "";
			// decorate the method name with the parameters data types to ensure uniqueness
			//List<ParameterToken> parameters = method.getParameters();
			ParameterToken[] parameters = method.getParameters().toArray(new ParameterToken[0]);

			for(int i = parameters.length -1; i >= 0; i--)
			{
				suffix+=parameters[i].getType();
			}
//			trace(" ** " + method.getName() + " - mapMethods suffix: " + suffix);
			m_map.put(method.getName() + suffix, method);//test.getParameters().size(), test);
		}
	}
